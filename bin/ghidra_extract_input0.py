...
class File_Extractor:
    ...
    def extract(self):
        ...
        function_label = open(os.path.join(self.output_dir, 'input0.label'), 'w')

        with open(os.path.join(self.stack_dir, f'input0_stack.json'), 'r') as f:
            loc_dict = json.loads(f.read())

        with open(os.path.join(self.input_dir, f'input0_code.json'), 'r') as f:
            code_dict = json.loads(f.read())
        
        for func in loc_dict.keys():

            opcodes = bytes(code_dict[func]['code'], encoding="raw_unicode_escape")
